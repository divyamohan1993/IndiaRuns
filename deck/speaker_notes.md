...
We evaluated three competing designs and grafted the best of each. Recall first-pass to 1,200 — and we measured the recall gate: 100% of proxy-Tier-5 and Tier-4 land in the shortlist, so nothing good is lost before the LLM sees it. LLM re-rank only on the shortlist, where 80% of the metric lives — 299 real judgments. Then a monotone blender; note we ship the safer one — the fixed-weight blend beat the learned model on cross-validation, so it ships. Behavioral multiplier modulates but never dominates. Honeypot gate at the end.
...
Read the table as receipts: 73.5 seconds against a 300-second cap, 2.01 GB against 16, no network, validator says valid, byte-identical determinism with the sha256, zero honeypots, and internal lift of about 1.0 NDCG@10 versus 0.07 for the naive baseline — more than a 10x lift on the metric. Then humanize it with the top-10: real product-company ranking and search engineers — Genpact AI doing LinkedIn RAG ranking at 50M queries a month, PharmEasy L2R search, CRED recsys, Flipkart prod RAG. These are genuine shippers, not keyword phantoms.
...
Be precise about boundaries. NVIDIA: offline embeddings and a 70B-class re-rank in Plane A only. LLM judging: 299 real judgments, 294 fact-validated reasoning lines, with a local CLI backend and a deterministic fallback so it runs with or without a key. GCP: Cloud Run and Secret Manager for production Plane C, documented and credential-gated, never needed to grade. The boundary: every AI call is in A or C; the graded rank.py makes zero — proven by a no-socket test and a network-none run.

### Slide 11 — Reproducibility (35s)
One command, sealed, twice. Show the docker run with --network none --cpus=4 --memory=16g — verified to produce a valid CSV offline. rank.py finishes in 73.5 s at 2.01 GB. Run it twice, byte-diff, identical — single-thread BLAS and a fixed hash seed make it deterministic. The validator runs inside rank.py and refuses to emit a bad file. Nothing to trust on faith; the grader reproduces the exact bytes.
